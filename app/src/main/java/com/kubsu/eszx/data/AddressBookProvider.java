...
import static com.kubsu.eszx.data.AddressBookDatabaseDescription.AUTHORITY;
...
public class AddressBookProvider extends ContentProvider {
    ...
    private static final String ONE_CONTACT_MIME_TYPE =
            "vnd.android.cursor.item/vnd.eszx.addressbook.data.contacts";
    private static final String CONTACTS_MIME_TYPE =
            "vnd.android.cursor.dir/vnd.eszx.addressbook.data.contacts";
    ...
    static {
        ...
        uriMatcher.addURI(AUTHORITY, AddressBookDatabaseDescription.Contact.TABLE_NAME + "/#", ONE_CONTACT);

        // Uri for Contacts table
        uriMatcher.addURI(AUTHORITY, AddressBookDatabaseDescription.Contact.TABLE_NAME, CONTACTS);
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case CONTACTS:
                ...
                long rowID = dbHelper.getWritableDatabase().insert(AddressBookDatabaseDescription.Contact.TABLE_NAME, null,
                        values);

                // if the contact was inserted, created an appropriate Uri;
                // otherwise, throw an exception
                if(rowID > 0) {
                    newContactUri = AddressBookDatabaseDescription.Contact.buildContactUri(rowID);

                    // notify observers that the database changed
                    Objects.requireNonNull(getContext()).getContentResolver().notifyChange(uri, null);
                } else {
                    throw new SQLException(
                            Objects.requireNonNull(getContext(), "Context cannot be null").getString(R.string.insert_failed) + uri);
                }
                break;
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        queryBuilder.setTables(AddressBookDatabaseDescription.Contact.TABLE_NAME);

        switch (uriMatcher.match(uri)) {

            case ONE_CONTACT: // contact with specified id will be selected
                queryBuilder.appendWhere(AddressBookDatabaseDescription.Contact.COLUMN_ID + "=" + uri.getLastPathSegment());
                break;
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case ONE_CONTACT:
                ...
                numOfRowsUpdated = dbHelper.getWritableDatabase().update(
                        AddressBookDatabaseDescription.Contact.TABLE_NAME, values, AddressBookDatabaseDescription.Contact.COLUMN_ID + "=" + id, selectionArgs);
                break;
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case ONE_CONTACT :
                ...
                numOfRowsDeleted = dbHelper.getWritableDatabase().delete(
                        AddressBookDatabaseDescription.Contact.TABLE_NAME, AddressBookDatabaseDescription.Contact.COLUMN_ID + "=" + id, selectionArgs);
                break;
